...
from SubscribeAssistantEnhancedPro import SubscribeAssistantEnhanced
from SubscribeAssistantEnhancedPro.engine.types import CompletionEvidence, CompletionSignal, PauseRecord
...
def test_release_metadata_requires_supported_main_program_version():
    """插件版本门禁要求使用具备全集洗版优先级合同的主程序版本。"""
    ...
    assert package["SubscribeAssistantEnhanced"]["system_version"] == ">=2.14.6"
    assert Version("2.14.5") not in specifier
    assert Version("2.14.6") in specifier
...
def test_disabling_site_total_probe_bulk_clears_all_leases_without_refresh_event():
    ...
    from SubscribeAssistantEnhancedPro.engine.site import SiteEvidence
...
def test_run_meta_check_calls_pause_when_pre_air_condition_holds():
    ...
    from SubscribeAssistantEnhancedPro.engine.types import PauseRecord
