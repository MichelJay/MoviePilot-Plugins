...
class PluginConfig:
    ...
    @property
    def subscription_cleanup_download_tasks(self) -> bool:
        """订阅清理下载任务：洗版完成后清理副下载器中的分集种子。"""
        return self.get_bool("subscription_cleanup_download_tasks", False)
    ...
    def best_version_type(self) -> str:
        """自动洗版类型范围：off=关闭, movie=仅电影, tv=仅剧集, all=电影和剧集。"""
        val = self.get_str("best_version_type", "off")
        return val if val in ("off", "movie", "tv", "all") else "off"

    @property
    def movie_downloader(self) -> str:
        """电影下载器：指定电影类型订阅使用的专用下载器，为空则使用默认下载器。"""
        val = self.get_str("movie_downloader", "")
        if not val:
            # backward compat: fall back to old best_version_downloader key
            val = self.get_str("best_version_downloader", "")
        return val

    @property
    def tv_downloader(self) -> str:
        """剧集下载器：指定剧集（全集）类型订阅使用的专用下载器，为空则使用默认下载器。"""
        val = self.get_str("tv_downloader", "")
        if not val:
            val = self.get_str("best_version_downloader", "")
        return val

    @property
    def tv_episode_downloader(self) -> str:
        """剧集分集下载器：指定剧集（分集）类型订阅使用的专用下载器，为空则使用默认下载器。"""
        val = self.get_str("tv_episode_downloader", "")
        if not val:
            val = self.get_str("best_version_downloader", "")
        return val

    @property
    def is_best_version_active(self) -> bool:
        """自动洗版是否启用：best_version_type 非 off 即启用。"""
        return self.best_version_type != "off"
