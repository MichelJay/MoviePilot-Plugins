...
async function loadSummary(api) {
  if (!api) return null;
  try {
    return await api.get("plugin/SubscribeAssistantEnhancedPro/summary");
  } catch {
    console.warn("[SubscribeAssistantEnhancedPro] summary unavailable");
    return null;
  }
}
async function loadDownloaders(api) {
  if (!api) return null;
  try {
    return await api.get("plugin/SubscribeAssistantEnhancedPro/downloaders");
  } catch {
...
const configDefaults = {
  ...
  "subscription_cleanup_download_tasks": false,
  ...
  "movie_downloader": "",
  "tv_downloader": "",
  "tv_episode_downloader": "",
  "best_version_type": "off",
...
const groups = [
  ...
  { key: "downloader", title: "下载器设置", icon: "mdi-download-network-outline", summary: "按电影/剧集全集/剧集分集指定专属下载器" }
];
const fields = [
  {
    "key": "enabled",
  ...
  {
    "key": "subscription_cleanup_download_tasks",
    "label": "洗版完成后清理旧种子",
    "group": "cleanup",
    "kind": "switch",
    "hint": "洗版完成后自动清理所有下载器中的旧分集种子和文件",
    "risk": "danger"
  },
  {
  ...
  {
    "key": "movie_downloader",
    "label": "电影专用下载器",
    "group": "downloader",
    "kind": "select",
    "hint": "指定电影类型订阅使用的下载器，包含初始下载与洗版，为空则使用默认下载器"
  },
  {
    "key": "tv_downloader",
    "label": "剧集全集下载器",
    "group": "downloader",
    "kind": "select",
    "hint": "指定剧集（全集）类型订阅使用的下载器，包含初始下载与洗版，为空则使用默认下载器"
  },
  {
    "key": "tv_episode_downloader",
    "label": "剧集分集下载器",
    "group": "downloader",
    "kind": "select",
    "hint": "指定剧集（分集下载）类型洗版订阅使用的下载器，为空则使用默认下载器"
  },
  {
    ...
    "group": "bestVersion",
    "kind": "select",
    "hint": "选择哪些媒体类型的订阅在下载完成后自动创建洗版订阅，关闭则不创建",
    "options": [
      { "title": "关闭", "value": "off" },
...
const messages = {
  "zh-CN": {
    ...
    "section.downloaderSettings": "下载器设置"
  },
  "zh-TW": {
    "config.changes": "本次修改",
    ...
    "section.custom": "自訂規則",
    "section.downloaderSettings": "下載器設定"
  },
  "en-US": {
    ...
    "section.downloaderSettings": "Downloader settings"
  }
};
...
const groupTranslations = {
  ...
  downloader: { tw: ["下載器設定", "按電影/劇集全集/劇集分集指定專屬下載器"], en: ["Downloader", "Assign dedicated downloaders by media type: movies, full seasons, and individual episodes"] }
};
const englishFields = {
  ...
  subscription_cleanup_download_tasks: ["Clean up old torrents after upgrade", "Automatically remove old episode torrents from all downloaders after upgrade completes"],
  ...
  movie_downloader: ["Movie downloader", "Specify a downloader for movie subscriptions (initial download + upgrades); leave empty for the default downloader"],
  tv_downloader: ["TV full-season downloader", "Specify a downloader for full-season TV subscriptions (initial download + upgrades); leave empty for the default downloader"],
  tv_episode_downloader: ["TV episode downloader", "Specify a downloader for episode-level TV upgrade subscriptions; leave empty for the default downloader"],
  best_version_type: ["Best-version scope", "Choose which media types auto-create upgrade subscriptions when download completes; Off disables it"],
...
const _sfc_main = /* @__PURE__ */ _defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const sectionDefinitions = {
      ...
      cleanup: [
        ...
        },
        {
          ...
          keys: ["subscription_cleanup_history_type", "subscription_cleanup_history_scenes", "subscription_cleanup_download_tasks"]
        }
      ],
      pending: [
        {
      ...
      downloader: [
        {
          titleKey: "section.downloaderSettings",
          keys: [
            "movie_downloader",
            "tv_downloader",
            "tv_episode_downloader"
    ...
    onMounted(() => {
      ...
      void loadDownloaders(props.api).then((options) => {
                        ...
                        return _openBlock(), _createElementBlock("div", {
                          ...
                          _createElementVNode("div", _hoisted_22, [
                            field.key === "reset_task" ? (_openBlock(), _createBlock(_component_VCheckbox, {
                              ...
                              density: "compact",
                              "hide-details": ""
                            }, null, 8, ["id", "modelValue", "onUpdate:modelValue", "aria-label"])) : field.group === "downloader" ? (_openBlock(), _createBlock(_component_VSelect, {
                              key: 2,
                              modelValue: _unref(draft)[field.key],
                              ...
                              "item-title": "title",
                              "item-value": "value",
                              items: [{ title: "使用默认下载器", value: "" }, ...downloaderOptions.value],
                              variant: "outlined"
                            }, null, 8, ["modelValue", "onUpdate:modelValue", "aria-label", "items"])) : field.kind === "select" || field.kind === "multi-select" ? (_openBlock(), _createBlock(_component_VSelect, {
                              key: 3,
                              modelValue: _unref(draft)[field.key],
                              "onUpdate:modelValue": ($event) => _unref(draft)[field.key] = $event,
                              "aria-label": field.label,
...
const Config = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-063f1fa3"]]);
