...
class SubscribeAssistantEnhanced(_PluginBase):
    ...
    # 插件作者
    plugin_author = "MichelJay"
    # 作者主页
    author_url = "https://github.com/MichelJay"
    # 插件配置项ID前缀
    ...
    def init_plugin(self, config: dict = None):
        ...
        logger.info(
            ...
            f"自动洗版={cfg.is_best_version_active} 下载管理={cfg.download_monitor_enabled} "
    ...
    def _init_modules(self):
        ...
        orchestrator = BestVersionOrchestrator(
            ...
            movie_downloader=cfg.movie_downloader,
            tv_downloader=cfg.tv_downloader,
            tv_episode_downloader=cfg.tv_episode_downloader,
        )
        subscription_cleanup = SubscriptionCleanup(
            ...
            cleanup_download_tasks=cfg.subscription_cleanup_download_tasks,
        )
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if cfg.is_best_version_active and cfg.best_version_cron:
    ...
    def run_best_version_check(self):
        ...
        if self._config and not self._config.is_best_version_active:
            return
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        ...
        return [{
            ...
            "path": "/downloaders",
            "endpoint": self._api_downloaders,
            "methods": ["GET"],
            "auth": "bear",
            "summary": "订阅助手（增强版）下载器列表",
            "description": "返回MoviePilot中注册的下载器列表",
        }]
    ...
    def _api_summary(self) -> Dict[str, Any]:
        ...
        return {
            "domains": {
                ...
                "自动洗版": cfg.is_best_version_active,
    ...
    def _api_downloaders(self) -> List[Dict[str, str]]:
        """返回MoviePilot中注册的下载器列表。"""
        downloaders = []
        if self._downloader_helper:
            services = self._downloader_helper.get_services()
            if services:
                for name in services.keys():
                    downloaders.append({"title": name, "value": name})
        return downloaders
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        if self._downloader_helper:
            ...
            if services:
                for name in services.keys():
                    downloader_choices.append({"title": name, "value": name})

        return build_form(downloader_choices=downloader_choices)
