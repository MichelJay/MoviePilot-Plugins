...
LABELS = {
    ...
    "subscription_cleanup_download_tasks": "洗版完成后清理旧种子",
    # 识别增强
    ...
    # 订阅洗版
    "best_version_type": "洗版范围",
    "movie_downloader": "电影专用下载器",
    "tv_downloader": "剧集全集下载器",
    "tv_episode_downloader": "剧集分集下载器",
...
HINTS = {
    ...
    # 订阅洗版
    "best_version_type": "选择哪些媒体类型的订阅在下载完成后自动创建洗版订阅，关闭则不创建",
    "movie_downloader": "指定电影类型订阅使用的下载器，包含初始下载与洗版，为空则使用默认下载器",
    "tv_downloader": "指定剧集（全集）类型订阅使用的下载器，包含初始下载与洗版，为空则使用默认下载器",
    "tv_episode_downloader": "指定剧集（分集下载）类型洗版订阅使用的下载器，为空则使用默认下载器",
...
TABS = [
    ("订阅清理", [
        ...
        ["subscription_cleanup_download_tasks"],
    ...
    ("订阅洗版", [
        ["best_version_type"],
        ["best_version_movie_remaining_days", "best_version_tv_remaining_days"],
        ["best_version_episode_to_full", "best_version_backfill_enabled", "backfill_best_version_now"],
    ]),
    ("下载器设置", [
        ["movie_downloader", "tv_downloader"],
...
def _field(key: str, defaults: dict, md: int = FIELD_MD, downloader_choices: list = None) -> dict:
    ...
    if key in ("movie_downloader", "tv_downloader", "tv_episode_downloader") and downloader_choices:
        return select_field(key, label, downloader_choices, hint, md)
...
def _tab_windows(defaults: dict, downloader_choices: list = None) -> list:
    ...
    for title, rows in TABS:
        ...
        for row in rows:
            ...
            for item in row:
                ...
                cols.append(_field(key, defaults, md, downloader_choices))
...
def build_form(downloader_choices: list = None):
    ...
    windows = _tab_windows(defaults, downloader_choices)
