...
class SubscriptionCleanup:
    ...
    def __init__(self,
                 ...
                 cleanup_download_tasks: bool = False):
        ...
        self._cleanup_download_tasks = cleanup_download_tasks
    ...
    def handle_resource_download_history_clear(self, subscribe, context=None, episodes=None) -> bool:
        ...
        if self._cleanup_download_tasks:
            self._clear_downloader_tasks(subscribe, old_hashes)
    ...
    def _clear_downloader_tasks(self, subscribe, download_hashes: set[str]):
        """清理旧的分集种子和文件（跨下载器删除）。

        逻辑：遍历所有下载器，根据 download_hashes 删除种子、辅种、以及文件资源。
        """
        if not download_hashes:
            logger.info(
                f"订阅清理：{format_subscribe_desc(subscribe)} 没有需要删除的旧种子"
            )
            return
        ...
        if not services:
            logger.info("订阅清理：没有注册的下载器")
            return

        logger.info(
            f"订阅清理：开始清理旧种子，下载器数量={len(services)}，待删除hash数量={len(download_hashes)}"
            f"（订阅={format_subscribe_desc(subscribe)}）"
        )
        ...
        for service_name, service in services.items():
            if not service or not service.instance:
                logger.info(f"订阅清理：下载器 {service_name} 不可用，跳过")
                continue

            try:
                success = service.instance.delete_torrents(
                    delete_file=True,
                    ids=list(download_hashes)
                )

                if success:
                    logger.info(
                        f"订阅清理：下载器 {service_name} 删除成功，删除 {len(download_hashes)} 个种子"
                    )
                    deleted_count += len(download_hashes)
                else:
                    logger.warning(
                        f"订阅清理：下载器 {service_name} 删除失败，尝试删除 {len(download_hashes)} 个种子"
                    )

            except Exception as err:
                logger.warning(
                    f"订阅清理：下载器 {service_name} 删除异常：{err}"
                )

        logger.info(
            f"订阅清理：清理完成，共尝试在 {len(services)} 个下载器中删除 {len(download_hashes)} 个种子"
            f"（订阅={format_subscribe_desc(subscribe)}，成功删除={deleted_count}）"
        )
